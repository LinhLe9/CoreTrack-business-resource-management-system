...
import { ProductionTicketFilter, ProductionTicketCardResponse } from '../../../types/productionTicket';
...
const ProductionTicketPage: React.FC = () => {
  ...
  const [queryParams, setQueryParams] = useState<ProductionTicketFilter>({});
  ...
  // Fetch production tickets with filter/search
  const fetchProductionTickets = useCallback(async (params: ProductionTicketFilter) => {
    try {
      const data = await getProductionTickets(params);
      setPageData(data);
    } catch (err: any) {
  ...
  }, [getProductionTickets, toast]);

  useEffect(() => {
    fetchProductionTickets(queryParams);
  }, [fetchProductionTickets, queryParams]);
  ...
  const handleSelectFromAutocomplete = (ticketId: number) => {
    ...
    router.push(`/production/${ticketId}`);
  };
        ...
        {!loading && !error && pageData && (
          <>
            ...
            <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={6}>
              {pageData.content.map((productionTicket) => (
                <ProductionTicketCard 
                  key={productionTicket.id} 
...
export default ProductionTicketPage;
